...
이 구조상 Codex CLI가 실행 환경에 설치되어 있어야 하며, 로컬 실행 시 호스트의 `~/.codex` 디렉토리에 인증 정보가 존재해야 합니다. Docker 환경에서는 호스트 `~/.codex`를 읽기 전용으로 참조하고, 인증/설정만 컨테이너의 writable `CODEX_HOME`으로 복사해 세션/모델 캐시는 Docker 볼륨에 기록하도록 분리했습니다.
...
질문 입력 → [L1 Exact Cache] → Hit → 즉시 반환
         ...
         [병렬] BM25 검색 + Vector 검색 (asyncio.gather, return_exceptions=True)
                  ↓
             RRF 융합 (가중치 0.3 / 0.7, k=60) + hybrid 임계값 필터 (0.007)
                  ↓
         [degraded mode] 단일 검색기면 상위 3개만 보수적으로 전달
                  ↓
             Top-K 청크 선택
                  ↓
             LLM 호출 (codex exec)
...
`RecursiveTextChunker`는 한국어 문서 특성을 고려해 문단 → 문장 → 어절 순으로 분리 시도합니다. `SEPARATORS = ["\n\n", "\n", ". ", "。", " ", ""]`로 우선순위를 뒀고, 청크 크기 512자 / 오버랩 50자로 문맥 연속성을 보장합니다. 현재 구현은 토큰 기준이 아니라 문자열 길이(`len`) 기준으로 동작합니다.
...
- Vector 검색 실패 → BM25 단독 모드로 degrade, hybrid min score는 적용하지 않고 상위 몇 개만 LLM에 전달
- BM25 검색 실패 → Vector 단독 모드로 degrade, 동일하게 bounded top-N 전략 사용

인프라 한쪽 장애가 서비스 전체 장애로 번지지 않게 격리하는 **결함 격리(Fault Isolation)** 원칙의 구현입니다. 또한 단일 검색기로 degrade된 상황에서는 hybrid 전용 임계값을 그대로 재사용하지 않도록 분리해, 장애가 곧바로 "관련 문서 없음"으로 오인되지 않게 했습니다.
...
- 근거가 있는 positive 응답만 저장, no-content 응답은 exact cache만 저장
```
...
if has_relevant_content and not partial_failure:
    asyncio.create_task(
...
Semantic cache는 근거가 확인된 positive 응답에만 적용합니다. no-content 응답까지 의미 기반으로 일반화하면, 한 번의 miss가 인접 질문들에 false negative로 재사용될 수 있기 때문입니다.
...
1. **외부 의존성은 실패할 수 있다고 보고 대비한다**: ChromaDB, Redis, Codex CLI처럼 서비스 바깥에 있는 구성요소는 실패 가능성을 전제로 보고, 가능한 범위에서 fallback 또는 예외 처리 경로를 두었습니다. 캐시 장애가 곧바로 전체 서비스 장애로 번지지 않도록 분리한 것도 같은 맥락입니다.
2. **예외는 가능한 한 구체적인 타입 기준으로 처리한다**: `except Exception`으로 모든 오류를 한 번에 삼키기보다, 의도한 예외만 분기하고 예상하지 못한 오류는 상위로 전파되게 했습니다.
3. **트레이드오프와 선택 이유는 문서로 설명한다**: 왜 이런 구조를 택했는지, 현재 한계가 무엇인지 같은 설계 맥락은 README와 이 문서에 남겨 평가자가 구현 의도를 빠르게 따라올 수 있게 했습니다.
4. **인프라 의존성은 느슨하게 연결한다**: 주요 인프라 구성요소는 DI로 주입해 서비스 레이어가 특정 구현에 과하게 묶이지 않도록 구성했습니다.
5. **문제 추적이 가능한 형태로 실패를 남긴다**: 주요 실패 경로에는 로그를 남겨, 운영 중 어떤 문제가 얼마나 자주 발생하는지 확인할 수 있게 했습니다.
