...
구현상으로는 `asyncio.create_subprocess_exec`로 stdout / stderr를 분리 캡처하고, stderr는 잘라서 로깅합니다. CLI가 websocket을 우선 시도했다가 HTTP로 fallback하는 경우도 stderr에 남지만, stdout의 최종 응답이 유효하면 서비스 응답은 정상 처리됩니다.
...
Codex CLI 역시 subprocess 기반 호출이라 Python 서비스가 직접 토큰 스트림 소켓을 제어하지는 않습니다. 따라서 세밀한 재시도보다 **CLI가 제공하는 재연결 / fallback 동작을 신뢰하고, 서비스는 stdout 파싱과 stderr 관측에 집중**하는 방향으로 설계했습니다.

운영 환경에서는 다음을 추가 고려할 수 있습니다:
- `asyncio.wait_for`로 전체 generator 소비에 wall-clock timeout 적용
- websocket fallback / 재연결 warning 빈도를 메트릭으로 수집해 CLI 버전 업데이트 시점 판단
