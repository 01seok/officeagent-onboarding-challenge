...
```bash
# 사전 요구: Python 3.12+, Redis, Node.js (claude CLI용)
brew install redis
...
API 서버는 `http://localhost:8000`에서 가동됩니다. Claude Pro/Max 구독 기반 인증이 로컬 `~/.claude`에 설정되어 있어야 LLM 호출이 정상 동작합니다.
...
| LLM 답변 생성 | claude-code-sdk 기반, 출처(Source) 추적 + 할루시네이션 억제 |
...
| LLM | claude-code-sdk | Claude Pro/Max 구독 기반 |
